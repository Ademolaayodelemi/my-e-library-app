 ...
 */
export const mySendEmail = async ({ email, subject, message }: { email: string; subject: string; message: string; }) => {
  await qstashClient.publishJSON({
    ...
    body: {
      from: "Bookish <lanre@larrytechcity.co.uk>", // Sender details
      to: [email], // Recipient list (array). "to" field expects an array of recipients
